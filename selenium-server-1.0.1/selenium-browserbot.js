 ...
 */
	...
	window.initIE9Support = function(bot){
		...
		if (browserVersion.ie9) {
			window.EventTrigger = {
				evt: [],
				setEventHandler: function(name, eventhandler) {
					if (name && name.length) {
						for (var i = 0, len = name.length; i < len; ++i) {
							this.evt[name[i]] = eventhandler;
						}
					} else 
						this.evt[name] = eventhandler;
				},
				trigger: function(name, args) {
					if (browserVersion.ie9) {
						if (name == EVENTS.click) {
						//					this.trigger(EVENTS.mousedown,[args[0],"mousedown",true,0,0]);
						//					this.trigger(EVENTS.mousedown,[args[0],"mouseup",true,0,0]);
						}
						this.evt[name].apply(this.evt[name], args);
					} else 
						triggerEvent.apply(triggerEvent, args);
				}
			};
			var EVENTS = {
				focus: "focus",
			...
			EventTrigger.setEventHandler(
				...
				function(element, eventType, keySequence, canBubble, controlKeyDown, altKeyDown, shiftKeyDown, metaKeyDown) {
					...
					try{
		            	evt.charCode = parseInt(keycode,10);
			...
			triggerEvent = triggerKeyEvent = function(element, eventType, canBubble, controlKeyDown, altKeyDown, shiftKeyDown, metaKeyDown) {
				try {
					EventTrigger.trigger(eventType, arguments);
				} 
				catch (ex) {
				}
			};
...
BrowserBot.prototype.triggerMouseEvent = function(element, eventType, canBubble, clientX, clientY, button) {
	if(browserVersion.ie9){
		return EventTrigger.trigger(eventType,arguments);
	}
    clientX = clientX ? clientX : 0;
    clientY = clientY ? clientY : 0;
    ...
    if (element.fireEvent && element.ownerDocument && element.ownerDocument.createEventObject) { //IE
        ...
        if (evt.initMouseEvent)
        {
			...
			evt.initMouseEvent(eventType, canBubble, true, document.defaultView, 1, screenX, screenY, clientX, clientY, this.controlKeyDown, this.altKeyDown, this.shiftKeyDown, this.metaKeyDown, button ? button : 0, null);
